...
class SarsaAgent:
    ...
    def get_best_policy_action(self, state):
        ...
        best_action = max(possible_actions, key=lambda x: self.get_q_value(state, x))
    ...
    def get_action(self, state):
        ...
        best_action = self.get_best_policy_action(state)
        b = np.random.random()
        if b > self.epsilon:
            return best_action
        else:
            ...
            l_ = len(action_list)
            return np.random.choice(action_list, p=[1 / l_] * l_)
    ...
    def update(self, state, action, next_state, reward, next_action):
        ...
        """
        update_value = self.alpha * (reward + self.gamma * self.get_q_value(next_state, next_action)) \
                       + (1 - self.alpha) * self.get_q_value(state, action)
        self.set_q_value(state, action, update_value)
