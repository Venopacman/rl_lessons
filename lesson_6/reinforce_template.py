...
def test_predict_proba(agent):
    ...
    assert tuple(test_probas.shape) == (test_states.shape[0], n_actions), "wrong output shape: %s" % np.shape(
        test_probas)
    assert np.allclose(np.sum(test_probas, axis=1), 1), "probabilities do not sum to 1"
    print('Test: predict_proba() function: OK!')


def test_generate_session(agent, env):
...
class ReinforceAgent(nn.Module):
    def __init__(self, state_dim, n_actions):
        ...
        self.fc5 = nn.Linear(50, n_actions)
...
def predict_proba(agent, states):
    ...
    probs = F.softmax(agent(torch.FloatTensor(states)), dim=1)
...
def generate_session(env, agent, t_max=1000):
    ...
    for t in range(t_max):
        ...
        action_probas = predict_proba(agent, [s])[0]
...
def get_cumulative_rewards(rewards,  # rewards at each step
    ...
    """
    def get_discount_return(rewards):
        if len(rewards) > 1:
            return rewards[0] + gamma * get_discount_return(rewards[1:])
        else:
            return rewards[0]
    cumulative_rewards = [get_discount_return(rewards[i:]) for i in range(len(rewards))]
...
# < YOUR CODE HERE >
def train_on_session(optimizer, agent, states, actions, rewards, gamma=0.99):
    """
...
if __name__ == '__main__':
    ...
    test_predict_proba(agent)
    test_generate_session(agent, env)
    ...
    for i in range(100):
        ...
        rewards = [train_on_session(optimizer, agent, *generate_session(env, agent)) for _ in range(100)]  # generate new sessions
        # optimizer.step()
