...
def update_policy(elite_states, elite_actions, n_states, n_actions):
    ...
    for state_iter in range(len(new_policy)):
        sum_ = np.sum(new_policy[state_iter])
        if sum_ > 0:
            new_policy[state_iter] = new_policy[state_iter] / sum_
        else:
            new_policy[state_iter] = np.ones(6)/6
...
def rl_cross_entropy():
    ...
    n_sessions = 600  # generate n_sessions for analysis
    percentile = 70  # take this percentage of 'elite' states/actions
